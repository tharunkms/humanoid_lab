...
class Classifier:
    ...
    def _probs(self, crop_bgr):
        """Softmax over the whole label vocabulary for one crop (numpy 1-D)."""
        import torch
    ...
    def classify_full(self, crop_bgr):
        """(label, shape, confidence, probs) -- probs is aligned with self.labels
        so detect_all can score a candidate against a user-chosen target
        category without a second CLIP pass."""
        probs = self._probs(crop_bgr)
        idx = int(probs.argmax())
        label = self.labels[idx]
        confidence = float(probs[idx])
        shape = CLIP_SHAPE_LABELS[label]
        return label, shape, confidence, probs

    def target_index(self, target_label):
        """Index of a vocabulary label, or None if it isn't one we know."""
        try:
            return self.labels.index(target_label)
        except ValueError:
            return None
...
class PropDetector:
    ...
    def detect(self, frame_bgr, classifier, target_label=None):
        ...
        img_area = float(h * w)
        target_idx = classifier.target_index(target_label) if target_label else None
        ...
        for seg, (x, y, bw, bh) in kept:
            ...
            if crop.size == 0:
                continue
            label, shape, conf, probs = classifier.classify_full(crop)
            if shape == "background":
                n_background += 1
                top2 = np.argsort(probs)[::-1][:2]
                alt = ", ".join(f"{classifier.labels[i]}={probs[i]:.2f}" for i in top2)
            ...
            if target_idx is not None:
                # how strongly THIS crop looks like the user's chosen
                # category, independent of what the argmax label was
                cand["target_prob"] = round(float(probs[target_idx]), 3)
...
class PointCloudBuilder:
    ...
    def fuse_session(self, session_dir, intr, poses=None):
        """
        ...
        # mask leakage in one view, or a reflection artifact). Keep only
        # the largest connected component to drop those entirely.
        if len(merged.points) > 20:
            labels = np.array(merged.cluster_dbscan(eps=self.voxel_size * 4, min_points=5))
            if labels.max() >= 0:  # at least one real cluster found (not all noise)
                sizes = np.bincount(labels[labels >= 0])
                largest = sizes.argmax()
                merged = merged.select_by_index(np.where(labels == largest)[0])
